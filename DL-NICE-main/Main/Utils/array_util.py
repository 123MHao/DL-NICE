...
def squeeze_2x2(x, reverse=False, alt_order=False):
    ...
    if alt_order:
        ...
        if reverse:
            ...
            # z = F.conv_transpose2d(x, perm_weight, stride=2)
            # torch.all(torch.eq(y.data,z.data)) # tested on 2020-06-30,by Levis, the result is equal
    else:
        b, c, h, w = x.size()
        x = x.permute(0, 2, 3, 1)
...
def checkerboard_mask(height, width, reverse=False, dtype=torch.float32,
    ...
    have the same value. In non-reversed (reverse=False, default) mask, top-left entry is 0.
